...
A complete activity discovery app with a React frontend, Fastify API, and SQLite database. Browse 185 seeded activities, personalize your mood, swipe on ideas, save matches, share invitations, and manage your account and API keys.
...
Open **http://localhost:3000**. Fastify serves the built frontend and `/api` from the same origin, including page reloads and public invitation URLs. Run these commands from the project root. `PORT` changes the production port; the development proxy uses port 3000.
...
Passwords use salted scrypt hashes. API keys are stored as SHA-256 hashes in the database. Sign-in credentials are stored in browser local storage; use HTTPS for hosted deployments. Browser-session keys issued by password sign-in are revoked on sign-out; personal API keys remain active until explicitly revoked.
...
Higgsfield image generation was attempted for this enhancement but rejected because the connected account requires a Basic plan or higher. No generated Higgsfield media is included; the shipped effects animate the existing frontend artwork.
